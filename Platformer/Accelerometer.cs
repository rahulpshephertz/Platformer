...
namespace Platformer
{
    ...
    public static class Accelerometer
    {
        ...
        public static void Initialize()
        {
            ...
            //if (System.Device.Location. == Microsoft.Devices.DeviceType.Device)            
            //{
            try
            {
                accelerometer.Stop();
                //accelerometer.CurrentValueChanged -= new EventHandler<Microsoft.Devices.Sensors.AccelerometerReadingEventArgs>(sensor_ReadingChanged);
        ...
        private static void accelerometer_CurrentValueChanged(object sender, Microsoft.Devices.Sensors.SensorReadingEventArgs<Microsoft.Devices.Sensors.AccelerometerReading> e)
        {
            // store the accelerometer value in our variable to be used on the next Update
            lock (threadLock)
            {
                nextValue = new Vector3((float)e.SensorReading.Acceleration.X, (float)e.SensorReading.Acceleration.Y ,(float)e.SensorReading.Acceleration.Z);
            }
        }
        ...
        public static AccelerometerState GetState()
        {
            ...
            if (isActive)
            {
                //if (Microsoft.Devices.Sensors. == Microsoft.Devices.DeviceType.Device)
                //{
